...
void _dynArrSetCapacity(DynArr *v, int newCap)
{
    ...
    /* create new dynamic array with size newCap */
	TYPE * newData = (TYPE*)malloc(newCap * sizeof(TYPE));

    /* copy over existing data elements */
    int i;
    for(i = 0; i < v->size; i++)
    {
        newData[i] = v->data[i];
    ...
    /* delete old array, replace with new larger one */
    free(v->data);
    v->data = newData;
}
...
{
    /* if index outside range, do nothing */
...
void removeDynArr(DynArr *v, TYPE val)
{
    ...
    for(i = 0; i < v->size; i++)
    {
        ...
        if(v->data[i] == val) 
        {
            /* remove the element at that index, break loop */
            removeAtDynArr(v, i);
            break;
        }
    }
