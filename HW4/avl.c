...
struct AVLnode * _balance(struct AVLnode * current)
{
    ...
    /* if balance factor is less than -1, rotate right */
    if(cbf < -1)
    {
        /* check if double rotation is necessary */
        if(h(current->left->right) > h(current->left->left))
            current->left = rotateLeft(current->left);
        current = rotateRight(current);
    }
    /* otherwise, check if left rotation is necessary */
    else if (cbf > 1)
    {
        /* check for double rotation */
        if(h(current->right->left) > h(current->right->right))
            current->right = rotateRight(current->right);
        current = rotateLeft(current);
    }

    /* set height again, return node */
	setHeight(current);
	return current;
}
...
struct AVLnode * AVLnodeAdd(struct	AVLnode * current, TYPE newValue)
{
    ...
    /* otherwise, call recusive for next layer down based on
    comparison to current node's value */
    else
    {
